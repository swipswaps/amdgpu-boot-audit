...
# ALLOW-FAIL-REASON: arithmetic increment (( x++ )) exits 1 when
#   result is zero — this is bash arithmetic semantics, not a failure.
#   || true prevents set -e from aborting on a zero-result increment.
# Source (Tier 2): bash(1) — "(( expr )) exits 1 if expr evaluates to 0."
...
%wheel ALL=(ALL) NOPASSWD: /usr/sbin/dmidecode"
    ...
    if [[ -f "$SUDOERS_FILE" ]]; then
        ...
        if [[ $DRY_RUN -eq 1 ]]; then
            ...
            # SUPPRESS-REASON: visudo -c stderr on a valid file is empty;
            #   on an invalid file it prints the syntax error. We suppress
            #   it here because we act on the exit code only — if invalid,
            #   we remove the file and log the failure explicitly.
            # Source (Tier 2): visudo(8) man page — "-c: check-only mode."
...
WantedBy=graphical.target"
    ...
    if [[ $DRY_RUN -eq 1 ]]; then
        ...
        # SUPPRESS-REASON: systemctl daemon-reload stderr outside a
        #   graphical session produces "Failed to connect to bus: No such
        #   file or directory". The || warn branch shows this explicitly.
        # Source (Tier 2): systemctl(1) man page — user session bus.
        #   https://man7.org/linux/man-pages/man1/systemctl.1.html
        ...
        # SUPPRESS-REASON: systemctl enable stderr outside a graphical
        #   session is noise — the || fail branch shows the failure.
        # Source (Tier 2): systemctl(1) man page.
        #   https://man7.org/linux/man-pages/man1/systemctl.1.html
