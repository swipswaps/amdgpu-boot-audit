...
#   || true 2-3 lines below the ALLOW-FAIL-REASON comment.
        ...
        # Look back up to 8 lines — bash -c multi-line commands push the
        # suppression far below the justification comment block.
        local look_start=$(( lineno > 80 ? lineno - 80 : 1 ))
        local context
        # SUPPRESS-REASON: sed -n — same rationale as above.
        ...
        # If context window contains "command -v", the 2>/dev/null is inside
        # a guarded block where the tool may not exist — justified.
        # ALLOW-FAIL-REASON: grep exits 1 on no match — || true prevents abort.
        echo "$context" | grep -q "command -v" && justified=1 || true
...
#   never errors. 2>/dev/null suppresses impossible fd errors only.
        ...
        # WHAT: skip functions that ARE the logging mechanism (warn, log, pass,
#   fail, header, ok, info, save_cmd) — their body is the output.
        # WHY:  flagging warn() for not calling log() is a false positive;
#   warn() IS a log-equivalent function.
        local is_output_fn=0
        echo "$fn_name" | grep -qE '^(warn|log|pass|fail|ok|info|header|save_cmd|check_dep|safe_read|require_cmd)$' \
            && is_output_fn=1
        # ALLOW-FAIL-REASON: grep exits 1 when fn_name does not match — valid.
        if echo "$fn_body" | grep -qE \
            "sqlite3|cp |mv |rm |tee |echo.*>|git |gh |systemctl|grubby|dnf|apt"; then
            if [[ $is_output_fn -eq 0 ]] && ! echo "$fn_body" | grep -qE "log |echo \[|printf"; then
