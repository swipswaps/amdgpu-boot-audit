...
save_cmd() {
    ...
    echo "------------------------------------------------------------"
    # SUPPRESS-REASON: save_cmd captures all stdout+stderr to a log file.
    #   2>&1 redirects stderr into the file alongside stdout — this is the
#   INTENDED behavior, not suppression. The output IS saved, not hidden.
    #   || true prevents set -e from aborting when a collector exits non-zero.
    # Source (Tier 2): bash(1) — "2>&1 redirects stderr to stdout."
#   https://man7.org/linux/man-pages/man1/bash.1.html
...
resolve_and_offer_install() {
    local cmd="$1"
    local reason="$2"
    log "ENTER resolve_and_offer_install: cmd=$1"
    ...
    if command -v dnf >/dev/null 2>&1; then
        ...
        # SUPPRESS-REASON: dnf -q stderr produces repo refresh noise and
#   "No matches found" messages that are noise when a tool is optional.
#   The empty pkg variable is handled by the [[ -z "$pkg" ]] branch below.
#   || true: dnf exits 1 on no-match — that is a valid result, not an error.
        # Source (Tier 2): dnf(8) man page — stderr on no-match.
...
safe_timeout() {
    ...
    # ALLOW-FAIL-REASON: timeout exits 124 when the command times out.
    #   That is the expected outcome for commands like systemd-analyze plot
    #   which may hang. The || true allows the script to continue.
    # Source (Tier 2): timeout(1) man page — exit status 124.
    #   https://man7.org/linux/man-pages/man1/timeout.1.html
...
for dep in "${DEPENDENCIES[@]}"; do
    # ALLOW-FAIL-REASON: require_cmd returns 1 for missing optional deps.
    #   The dependency loop continues — missing tools are warned, not fatal.
    # Source (Tier 2): bash(1) — exit status propagation in loops.
    #   https://man7.org/linux/man-pages/man1/bash.1.html
...
#   https://man7.org/linux/man-pages/man1/bash.1.html
# ALLOW-FAIL-REASON: resolve_and_offer_install returns 1 when a tool
#   is not in any enabled repo. || true allows the script to continue
#   without the optional tool — confirmed fix for original stall.
# Source (Tier 2): bash(1) — set -e and || true interaction.
#   https://man7.org/linux/man-pages/man1/bash.1.html
    ...
    for f in status enabled dpms modes edid connector_id power subsystem uevent; do
        if [[ -e "$d/$f" ]]; then
            ...
            if [[ "$f" == "edid" ]]; then
                ...
                # SUPPRESS-REASON: sysfs node may vanish on hotplug between
#   readdir and open. 2>/dev/null suppresses "No such file" race error.
#   || true prevents abort when the node disappears mid-read.
                # Source (Tier 2): sysfs(5) man page — transient nodes.
...
for f in \
    ...
    for p in $f; do
        [[ -e "$p" ]] || continue
        echo "==== $p ===="
        # SUPPRESS-REASON: sysfs nodes (/sys/class/drm/...) can
        #   vanish between readdir and open on hotplug events.
        #   The || true / || echo handles the race visibly.
        # Source (Tier 2): sysfs(5) man page — transient nodes.
        #   https://man7.org/linux/man-pages/man5/sysfs.5.html
...
# SUPPRESS-REASON: find produces "Permission denied" on sysfs nodes
#   that are root-only (e.g. /sys/class/drm/*/edid). Those errors are
#   noise — the found paths are what matters.
# Source (Tier 2): find(1) man page — stderr on permission errors.
#   https://man7.org/linux/man-pages/man1/find.1.html
find /sys/class/drm -name edid 2>/dev/null | while read -r edid; do
    safe_name="$(echo "$edid" | tr '/' '_')"
    # SUPPRESS-REASON: EDID sysfs node may vanish between find and cp
    #   on display hotplug. The || true handles the race visibly.
    # Source (Tier 2): sysfs(5) man page — transient nodes.
    #   https://man7.org/linux/man-pages/man5/sysfs.5.html
...
save_cmd initramfs-amdgpu.txt "AMDGPU presence in initramfs" \
    # SUPPRESS-REASON: lsinitrd stderr on non-dracut or missing initramfs
    #   is noise — the grep pipe handles empty output visibly.
    # Source (Tier 2): dracut(8) man page — lsinitrd tool.
...
classify() {
    ...
    log "ENTER classify: severity=$1 file=$4"
...

{
    ...
    # SUPPRESS-REASON: grub2-editenv stderr on non-GRUB2 systems is noise.
    #   The || echo fallback is shown explicitly.
    # Source (Tier 2): grub2-editenv(1) — GRUB environment block tool.
...
# SUPPRESS-REASON: sudo -n true stderr produces "sudo: a password is
#   required" when NOPASSWD not configured — that IS the check result,
#   represented by the exit code. The if/else handles both paths visibly.
# Source (Tier 2): sudo(8) man page — "-n: non-interactive mode."
#   https://man7.org/linux/man-pages/man8/sudo.8.html
...
SUMMARY="$OUTDIR/summary.txt"
{
    ...
    echo
    echo "Framebuffer:"
    # ALLOW-FAIL-REASON: /proc/fb is absent when no framebuffer device
    #   is registered (pure console boot, no KMS). || true allows the
    #   summary section to continue without the fb info.
    # Source (Tier 2): proc(5) man page — /proc/fb.
    #   https://man7.org/linux/man-pages/man5/proc.5.html
    cat /proc/fb || true
    echo
    echo "Classification:"
    # SUPPRESS-REASON: classification.txt may not exist if the engine
    #   found no matches (empty output is valid). The || echo makes that
    ...
    # SUPPRESS-REASON: shutdown-duration.txt absent if previous boot had
    #   no journal (e.g. first boot). The || echo makes that explicit.
    # Source (Tier 2): bash(1) — || as conditional execution.
    #   https://man7.org/linux/man-pages/man1/bash.1.html
